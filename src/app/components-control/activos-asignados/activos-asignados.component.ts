...
@Component({
  ...
  styleUrls: ['./activos-asignados.component.css']
})
...
ngOnInit() { 
  this.controlService.getBienesAsignados().subscribe(res=> { this.comboAreaSucur=res});
...
buscar(buscador) {
this.p = 1;
this.controlService.buscarActivoAsig(buscador.value).subscribe(res => {this.comboAreaSucur = res});
}
